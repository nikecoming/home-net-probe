...
sudo tshark -r "$M" -Y "tcp.analysis.retransmission && ($SP || $DP)" -T fields -e frame.len 2>>"$TSERR" \
...
echo
echo "== B. 對照：全部封包的大小分布 =="
sudo tshark -r "$M" -Y "tcp && ($SP || $DP)" -T fields -e frame.len 2>>"$TSERR" \
...
CT=$(sudo tshark -r "$M" -Y "tcp && ($FILT) && ($SP || $DP)" 2>>"$TSERR" | wc -l)
CR=$(sudo tshark -r "$M" -Y "tcp.analysis.retransmission && ($FILT) && ($SP || $DP)" 2>>"$TSERR" | wc -l)
KT=$(sudo tshark -r "$M" -Y "tcp && !($FILT) && ($SP || $DP)" 2>>"$TSERR" | wc -l)
KR=$(sudo tshark -r "$M" -Y "tcp.analysis.retransmission && !($FILT) && ($SP || $DP)" 2>>"$TSERR" | wc -l)
...
sudo tshark -r "$M" -Y "tcp.flags.syn==1 && ($SP || $DP)" -T fields -e ip.src -e ip.dst -e tcp.options.mss_val 2>>"$TSERR" \
 | awk 'NF==3 && $3!=""{k=$3; c[k]++} END{for(m in c) printf "   MSS %-6s  %d 次\n", m, c[m]}' | sort -k2 -rn | head
echo
echo "== E. ICMP 需分割 / 不可達（PMTUD 訊號）=="
sudo tshark -r "$M" -Y "icmp.type==3" -T fields -e ip.src -e icmp.code 2>>"$TSERR" | sort | uniq -c | sort -rn | head
[ -z "$(sudo tshark -r "$M" -Y 'icmp.type==3' 2>>"$TSERR")" ] && echo "   （無，代表沒有 PMTUD 回饋）"
)
echo "$OUT"
echo
echo "---- 上傳中 ----"
