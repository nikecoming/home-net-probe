...
c(){ tshark -r "$1" -Y "$2" 2>/dev/null | wc -l; }
...
if [ -n "$BEST" ] && [ "$BESTN" -gt 0 ]; then
  ...
  tshark -r "$BEST" -Y "tcp.flags.syn==1&&tcp.flags.ack==0&&$ME" -T fields \
     -e frame.time -e tcp.analysis.retransmission 2>/dev/null \
  ...
  tshark -r "$BEST" -Y "tcp.analysis.retransmission&&tcp.flags.syn==1&&tcp.flags.ack==0&&$ME" \
     -T fields -e ip.dst -e tcp.dstport 2>/dev/null | sort | uniq -c | sort -rn | head -14 | sed 's/^/   /'
  echo
  echo "   -- 該檔隧道 vs 直連 --"
